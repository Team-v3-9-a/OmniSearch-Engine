...
- Возвращает 404 Not Found если видео с таким ID не существует
...
#### UI удаления видео
**Описание:** Кнопка удаления на карточке видео и на странице видео с подтверждением. API: `DELETE /api/v1/videos/{id}` → 204 No Content (успех), 404 Not Found (нет такого видео).
